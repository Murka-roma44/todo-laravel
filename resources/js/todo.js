...
$(document).ready(function() {
       ...
       $.ajax({
          ...
          success: function (data) {
             ...
             if (data.data.is_completed==1) {
                completed = `<a class="btn btn-sm btn-success" href="">Completed</a>`
             } else {
               completed = `<a class="btn btn-sm btn-danger" href="">In completed</a>`
             }
             ...
             <tr id="${data.data.id}">
               ...
               <td id="outer">
                  <a class="inner btn btn-sm btn-success" href="/show/${data.data.id}">View</a>
                  <a class="inner btn btn-sm btn-info" href="/${data.data.id}/edit">Edit</a>
          ...
          success: function (data) {
            ...
            if (data.data.is_completed==1) {
              ...
              completed = `<a class="btn btn-sm btn-danger" href="">In completed</a>`
               ...
               document.getElementById("completed-"+id).innerHTML = completed;
               alert(data.message);
           },
           error: function (data) {
            alert(data.message);
   ...
   $('.filter-container').on('submit', 'form', function (e) { 
      ...
      var completed = t.find('select[name="completed"]').val()
       ...
       $.ajax({
          ...
          success: function (data) {
             ...
             data.data.forEach(data => {
                ...
                if ( users.includes(user_ids)) {
                   outer =`<a class="inner btn btn-sm btn-info" href="/${data.id}/edit">Edit</a>
                   <form action="/destroy" class="inner">
                      <input type="hidden" name="_token" value="${token}">
                      <input type="hidden" name="_method" value="DELETE">
                ...
                todo += `
                  ...
                  <td id="outer">
                     <a class="inner btn btn-sm btn-success" href="/show/${data.id}">View</a>
                     ${outer}
                  </td>
               </tr>
               `;
